...
書き換えたリクエストで、複数の認証をHTTP経由で呼び出す認証モジュールです。  
この認証は、呼び出した認証が全て成功した時だけ成功します。
...
### **\[and.response.invalid\_setting\]** 部分
